$(document).ready(function () {
  ...
  // Load our header
  $("#header").load(
    "http://{SERVER_URL}:3000/resources/handbook/content/header.html",
    function () {
      $.getScript(
        "http://{SERVER_URL}:3000/resources/handbook/scripts/header.js"
      );
    }
  );
  ...
  // Load our main page
  $("#content-area").load(
    "http://{SERVER_URL}:3000/resources/handbook/content/main.html",
    function () {}
  );
  ...
  $("html").on("click", "a", function (e) {
    if (!search.active) {
      ...
      $("#content-area").load(
        "http://{SERVER_URL}:3000/resources/handbook/content/" + page_name,
        function () {}
      );
      e.preventDefault();
    }
  ...
  document.onkeyup = (e) => {
    ...
    //    ToggleOpen();
    //}
    if (keycode == 13 && $("#search").is(":focus") && open && !search.active) {
      // if they press enter
      ShowSearchUI();
  ...
  $("html").on("click", "div.search-result", function () {
    if (!search.active && search.results.length > 0) {
      $("#content-area").load(
        search.results[$(this).attr("data-index")].name,
        function () {}
      );
    }
  });
    ...
    if (search.progress == search.total) {
      ...
      if (search.results.length > 0) {
        ...
        for (let i = 0; i < search.results.length; i++) {
          const name = search.results[i].html.substring(
            7,
            search.results[i].html.indexOf(`</title>`)
          );
          $("div.content").append(
            $(`<div data-index='${i}' class='search-result'></div>`).text(name)
          );
        }
      } else {
  ...
  $(window).on("resize", function () {
    $("div.content").css(
      "max-height",
      $("div.content").parent().height() - $("#header").height()
    );
  });
