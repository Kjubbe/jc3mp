...
function UpdateDatabase(player, t) {
  ...
  if (!inv.utils.exists(player) || !player.c || !player.c.inventory) {
    jcmp.events.Call(
      "log",
      "inventory",
      `Tried to UpdateDatabase for player with steam id 
  ...
  inv.pool
    ...
    .then(function (result) {
      if (result && result[0]) {
        // If their inventory exists
        sql = inv.sqlstring.format(
          "UPDATE inventory SET data = ? WHERE steam_id = ?",
          [inv_str, steam_id]
        );
      ...
      } // If their inventory does not exist
      else {
