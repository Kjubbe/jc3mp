...
jcmp.events.Add("inventory/events/use_item", (player, stack, index) => {
  const item =
    player.c.inventory.contents[stack.get_first().category][index].get_first();
  ...
  if (
    ...
    if (item.custom_data.drawing_id == undefined) {
      ...
      Promise.all([AddImageToDBOnCreate()]).then((data) => {
        const id = data[0];
...
jcmp.events.AddRemoteCallable(
  "itemuse/paper/mod_pen_dura",
  (player, pen_name, dura) => {
    ...
    if (item && index > -1) {
      ...
      if (dura < pen_stack.get_first().durability) {
        const broke = player.c.inventory.modify_durability(
          pen_stack,
          index,
          dura - pen_stack.get_first().durability
        );
        ...
        if (broke) {
          ...
          if (index2 > -1) {
            // If they have another one, update the durability
            jcmp.events.CallRemote(
              "itemuse/paper/update_pen_dura",
              player,
              pen_name,
              player.c.inventory.contents[item.category][index2].get_first()
                .durability
            );
          } // Otherwise, remove pen entry
          else {
...
function UpdateImage(player, id, image_data, name) {
  if (
    player.loaded_drawing != undefined &&
    String(player.loaded_drawing) == String(id)
  ) {
    if (
      name.length < 3 ||
    ...
    const sql = iu.sqlstring.format(
      "UPDATE drawings SET image_data = ?, name = ? WHERE drawing_id = ?",
      [image_data, name, id]
    );
      ...
      for (let i = 0; i < player.c.inventory.contents["Utility"].length; i++) {
        ...
        for (let i = 0; i < stack.contents.length; i++) {
          ...
          if (String(item.custom_data.drawing_id) == String(id)) {
            ...
            player.c.inventory.update();
            player.c.inventory.update_index(
              item_category,
              player.drawing_index
            );
          }
        }
      }
  ...
  if (images[String(id)] !== undefined) {
    ...
    iu.config.pens.forEach((pen) => {
      ...
      if (item && index > -1) {
        pens.push({
          name: pen,
          d: player.c.inventory.contents[item.category][index].get_first()
            .durability,
        });
      }
    });
    ...
    jcmp.events.CallRemote(
      ...
      player,
      String(id),
      String(images[String(id)].data),
      String(images[String(id)].name)
    );
    jcmp.events.CallRemote(
      "itemuse/paper/init_pens",
      player,
      JSON.stringify(pens)
    );
  } else {
    console.log(
      `Could not load image id ${id} for player ${player.c.general.steam_id}`
    );
  }
}
  ...
  iu.pool.query(sql).then((result) => {
    if (result.length > 0) {
      for (let i = 0; i < result.length; i++) {
        images[String(result[i].drawing_id)] = {
          data: String(result[i].image_data),
